...
export const getCountReport = async (req, res) => {
  const ownerId = req.user._id; // Get the owner ID from the request
  const isOwner = req.user.roleId === '676e3938d0f5a92c824fc662'; // Check if the user has the  Owner role
  try {
    const [
      activeMember,
      inActiveMember,
      ...
      Member.aggregate([
        {
          $match: isOwner ? { status: 1, createdById: new mongoose.Types.ObjectId(ownerId) } : { status: 1 }
        },
        {
          $count: 'activeMember'
        }
      ...
      Member.aggregate([
        {
          $match: isOwner ? { status: 0, createdById: new mongoose.Types.ObjectId(ownerId) } : { status: 0 }
        },
        {
          $count: 'inActiveMember'
        }
      ...
      Subscription.aggregate([
        {
          $lookup: {
            ...
            pipeline: [
              {
                $match: isOwner ? { createdById: new mongoose.Types.ObjectId(ownerId) } : {}
              }
            ]
          }
        },
      ...
      Subscription.aggregate([
        ...
        {
          $match: {
            endDate: { $lt: new Date(new Date().setDate(new Date().getDate() + 10)) },
            expInStatus: 1
          }
        },
        {
      ...
      ExpiredSub.aggregate([
        {
          $match: {
            status: 1,
            createdById: isOwner ? new mongoose.Types.ObjectId(ownerId) : {}
          }
        },
        {
          $count: 'expSubscriptionCount'
    ...
    res.status(200).json({
      ...
      message: 'Counts fetched successfully',
      data: {
        totalMember: (activeMember[0]?.activeMember || 0) + (inActiveMember[0]?.inActiveMember || 0),
        activeMember: activeMember[0]?.activeMember || 0,
        inActiveMember: inActiveMember[0]?.inActiveMember || 0,
        totalAmount: totalAmount[0]?.totalAmount || 0,
        totalPaidAmount: totalAmount[0]?.totalPaidAmount || 0,
        totalUnpaidAmount: (totalAmount[0]?.totalAmount || 0) - (totalAmount[0]?.totalPaidAmount || 0) - (totalAmount[0]?.totalDueAmount || 0),
        totalDueAmount: totalAmount[0]?.totalDueAmount || 0,
        expiredSub: expiredSub[0]?.expiredSub || 0,
        expiredSubsBeforeSevenDay: expiredSubsBeforeSevenDay[0]?.expiredSubsBeforeSevenDay || 0,
        expSubscriptionCount: expSubscriptionCount[0]?.expSubscriptionCount || 0
      }
    });
  } catch (error) {
...
export const getExpiredSubsBeforeSevenDay = async (req, res) => {
  const roleName = req.user.roleId
    ...
    const expiredSubsBeforeSevenDay = await Subscription.aggregate([
      {
        $lookup: {
          ...
          pipeline: [
            {
              $match: {
                ...(roleName =='67193213e0e76d08635e31fb'?{}:{createdById: new mongoose.Types.ObjectId(req?.user?._id)}),
                  // createdById: new mongoose.Types.ObjectId(req?.user?._id),
                  status: 1
              }
            }
          ]
...
export const monthWisePlanWiseReport = async (req, res) => {
  ...
  try {
    ...
    console.error('Error fetching monthwise and planwise report:', error);
    res.status(500).json({
      success: false,
      message: 'An error occurred while fetching the monthwise and planwise report.',
      error: error.message
    });
  }
};
